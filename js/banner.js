...
inc[el.id]=0;buttons=$('#ft-buttons-'+el.id).children();buttons.each(function(index){if(index==imgInc[el.id]){$(this).addClass('ft-button-'+el.id+'-active').css("background-position","bottom");}else{$(this).removeClass('ft-button-'+el.id+'-active').css("background-position","top");}});if(opts[el.id].direction=='random')
...
this.each(function(){init(this);});};$.fn.jqFancyTransitions.defaults={width:700,height:265,strips:20,delay:5000,stripDelay:50,titleOpacity:0.7,titleSpeed:1000,position:'alternate',direction:'fountainAlternate',effect:'zipper',navigation:false,links:false};})(jQuery);